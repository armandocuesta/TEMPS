...
only RUN this only if you want numeric data for the TEMPS
```{r}
tempsnum <- subset(tempsdata, select = -c(gender, race, diagnosis, psychotic))
tempsnum <- as.data.frame(lapply(tempsnum, as.numeric))
head(tempsnum)

tempsfac <- select(tempsdata, id, gender, race, diagnosis, psychotic)
temps <- full_join(tempsnum, tempsfac, by = "id")
```
...
```{r}
temps <- read.spss("J:\\psych\\Neurocognition Lab\\Heterogeneity\\Data_Heterogeneity\\TEMPS machine learning project\\Temps_impure.sav", 
           to.data.frame=TRUE)
...
# you may want to create a separate predictor stating
# if you have NA or not before you do NA imputation
...
Doing logistic regression (this does not work)
```{r}
